...
Our instrumentation that prevents this kind of attacks adds in average less than 
...
\paragraph{Grafo de dependências de dados. }
A figura~\ref{fig:tblDepGraph} mostra informações estáticas a respeito da
estrutura dos programas analisados.
Essa estrutura compreende o tamanho absoluto do programa, em número de 
instruções e as informações do grafo de dependências de dados extraído de cada 
programa.
É possível observar que em média 14\% dos valores podem conter informações 
vindas da entrada de dados. Esse numero é influenciado por todas as chamadas a 
funções externas ao código fonte avaliado, para as quais não há como saber se 
há ou não acesso a dados externos.
Outra informação importante é que há mais nós de operação no grafo do que instruções no programa.
Esse fato acontece pois nossa análise é interprocedural e é preciso inserir nós 
de operação para ligar parâmetros formais a parâmetros reais e também para ligar
valores de retorno aos valores que recebem o resultado de funções.
Além disso, a tabela apresenta a quantidade de nós de memória, que são vértices do grafo que
agrupam variáveis que contém ponteiros e que podem apontar
para a mesma região de memória. Para identificar quais variáveis pertencem a cada vértice de memória, 
é necessária a realização de uma análise de ponteiros seguida da criação de 
conjuntos disjuntos de áreas de memória.
Finalmente, o número de arestas dos grafos gerados indica que no caso médio, o grafo é esparso.
Nos programas do SPEC, a relação é de 1,38 arestas por vértice, em média.
Essa característica é evidenciada pela média de relação de 1,38 arestas por vértice, nos programas do SPEC; 
e pela correlação de XX entre o número de vértices e o número de arestas de cada grafo, considerando 
os 100 maiores casos de teste padrão do LLVM.
...
Benchmark 		& Num. Inst. 	& Entr.		& Num. Op. 		& Num. Var. 		& Num. Mem. 		& Num. Arestas 		\\ \midrule
...
{\bf Num. Inst.}: número de instruções do programa.
{\bf Entr.}: número de valores que podem representar entradas de dados.
...
Vemos, pela figura, que Aproximadamente 70\% de todos os laços do programa
são alcançáveis.
Aproximadamente a metade dos laços alcançáveis é vulnerável.
Dos laços vulneráveis, aproximadamente 12\% são controlados por comparações do 
tipo \texttt{i <= N};
Conforme vemos pela figura, os caminhos entre as entradas de dados e as condições 
de parada dos loops são geralmente pequenos.
Os dez caminhos vulneráveis em \texttt{429.mcf}, por exemplo, possuem somente
sete instruções, em média.
Conclui-se que o desenvolvedor, ao procurar por possíveis vulnerabilidades
...
\renewcommand{\arraystretch}{1.2}
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}lrrrrrr} \toprule
Benchmark			&	Num. Laços (L)	&	Alcançáveis	&	Caminhos	&	\multicolumn{2}{c}{Vulneráveis} &	\% (II/L) \\ 
                                                                     \cmidrule{5-6} 
{}				&	{}		&	{}			&	{}				&	I		&	II	&	{}	\\ \midrule
...
vulnerabilidade.
{\bf I}: análise considerando todas as condições de parada de loop potencialmente vulneráveis.
{\bf II}: análise considerando somente as condições de parada de loop controlados por comparações do tipo \texttt{i <= N}.}
\label{fig:staticLoopData}
\end{figure}



\paragraph{Instrumentação. }
A figura~\ref{fig:instImpact} mostra o impacto da instrumentação que inserimos
no tamanho do programa.
Em média, cada laço vulnerável custou-nos a criação de 1.7 guardas, como
...
Na média, entretanto, os programas cresceram apenas 5\% em número de instruções.
Uma vez que o número de instruções inseridas nos programas é tão pequeno, o
crescimento de seu tempo de execução também é pequeno, pois nenhuma das 
instruções realiza operações demoradas, como acesso a memórias lentas.
...
% XX - Melhorar esse final de parágrafo, colocando os resultados do teste dinâmico
...
\renewcommand{\arraystretch}{1.2}
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}lrrrrrr} \toprule
Benchmark		&	Num. Inst.	&	Num. Arit.	&	\multicolumn{2}{c}{Instrumentação}		&	\multicolumn{2}{c}{Crescimento}			\\ 
...
{\bf Num. Inst.}: número de instruções no benchmark.
