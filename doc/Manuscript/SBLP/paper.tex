...
\email{\{victorsc,igor.rafael,raphael.hernani,fernando\}@dcc.ufmg.br}
}
...
function.
However, we can gain in precision by performing it inter-procedurally; that
is, by allowing the results found for a function $f$ to flow into other
...
the same function.
Figure~\ref{context} shows way the ability to make this distinction is
...
The results that we can derive for the transformed program are much more
