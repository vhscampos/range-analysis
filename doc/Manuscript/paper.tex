...
Our main algorithmic contribution on top of previous works is a three-phases
...
aware of, such as those in Java Hotspot~\footnote{} and gcc~\footnote{} only
deal with comparisons between variables and constants.
Even the theoretical works, such as Su and Wagner's~\cite{Su05} or Gawlitza
...
programs.
Firstly, we use Bodik's Extended Static Single Assignment (e-SSA) form to
...
Secondly, we process the strongly connected components that underlines our
...
instance.
It is also precise: our results are similar to Stephenson {\em et al}'s,
even though, contrary to them, we do not assume that the program is correct.
...
{\em et al.}~\cite{Lakhdar11}, even though they use much more costly methods.
