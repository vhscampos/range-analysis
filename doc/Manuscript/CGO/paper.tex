...
stores the result in a variable $x$.
The semantics of a SUB instruction is $x = op_1 - op_2$. 
...
stores the result in a variable $x$.
The semantics of a MUL instruction is $x = op_1 \times{} op_2$. 
...
because the result of the MUL will be $0$, too. In this case, if $op_1 = 0$,
...
The semantics of a SHL instruction is $x = op_1 \times{} 2^n$. 
...
Wang {\em et a.}~\cite{Wang09} have implemented a tool, IntScope, that combines symbolic execution and taint analysis to detect integer overflow vulnerabilities. The authors have been able to use this tool to successfully identify many vulnerabilities in industrial quality software. Our work, and Wang {\em et a.}Õs work are essentially different: they use symbolic execution, whereas we rely on
range analysis.
