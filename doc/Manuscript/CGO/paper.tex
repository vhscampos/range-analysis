...
of these overflows.
In this paper we present the first algorithm that uses static range analysis to
avoid this instrumentation whenever possible.
Our range analysis contains novel techniques, such as a live range splitting
strategy that refines the precision of our results.
...
Firstly, in Section~\ref{sub:micro} we propose a three-phases approach to extract
...
However, given our three-phases approach, we improve precision
...
By carefully crafting an input file, an adversary can, in this way, overwrite
...
If $V$ is used by $C$ as the input of a future range, then the edge from
...
We determine the ranges for each strongly connected component in three phases.
...
In addition, we have not been able to collect the statistics regarding
the overflows occurred in SPEC's \texttt{bzip2}, because the log file
has became too large, due to the occurrence of more than 3.000.000.000 
overflows. Thus, the machine ran out of memory.
...
Out of all the 102,790 instructions that we have instrumented in SPEC, 3.92\% are
suspicious, 17.19\% are safe, and
78.89\% are uncertain.
...
Comparison between execution times with and without prunning, 
...
There exists other variants of SSU~\cite{Plevyak96,George03}.
