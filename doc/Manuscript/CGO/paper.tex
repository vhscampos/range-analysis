...
Thus, the instrumented program can take some action when an overflow happens,
such as to log the event, or to terminate the program.
However, this safety has a price: arithmetic operations need to be
surveilled, and the runtime checks cost time.
Zhang {\em et al.}~\cite{Zhang10} have eliminated some of this
...
Firstly, in Section~\ref{sub:prop} we show how we rely on strongly connected
...
Yet, we go beyond: given our three-phase approach, we improve precision
by solving strong components in topological order.
Secondly, in Section~\ref{sub:micro} we describe this three-phase approach to
extract information from comparisons between variables, e.g., $x < y$.
...
This instrumentation framework, which we describe in
...
from this program.
There is a correspondence between instructions and constraints.
Our analysis is sparse~\cite{Choi91}; thus, it associates one, and only one,
constraint with each integer variable.
...
Firstly, we convert the program to a representation that gives
...
We let $\bot$ denote non-initialized intervals, so that $[\bot, \bot] \sqcup
[l, u] = [l, u]$.
...
the operation that truncates to $n$ bits.
...
Number of instructions used in each check.}
\end{center}
\end{figure}
...
user, who has the option to overwrite the \texttt{handle\_overflow}
...
Figure~\ref{fig:TimeCorr}: the coefficient of determination is 0.994.
...
Wang {\em et al.}~\cite{Wang09} have implemented a tool, IntScope, that combines symbolic execution and taint analysis to detect integer overflow vulnerabilities. The authors have been able to use this tool to successfully identify many vulnerabilities in industrial quality software. Our work, and Wang {\em et al.}'s work are essentially different: they use symbolic execution, whereas we rely on
range analysis.
...
\noindent
\textbf{Software: } Our implementation is
publicly available at \\
\texttt{http://code.google.com/p/range-analysis/}.

\acks
We thank Douglas do Couto and Igor Costa for helping us with the LLVM code
base.
We also thank the anonymous reviewers for comments and
suggestions.
This project is supported by FAPEMIG and Intel.
