...
def floodFirst(Valuation, Constraint, source, Done):
  ...
  for (u, v) in Constraint.keys():
    if u == source and not Done.__contains__(v):
      ...
      Done.add(v)
      floodFirst(Valuation, Constraint, v, Done)

def floodSecond(Valuation, Constraint, source, seed, Done):
  """Propagate a final interval to every node in V, starting from source."""
  for (u, v) in Constraint.keys():
    if u == source and not Done.__contains__(v):
      Valuation[v] = Valuation[v].union(Constraint[(u, v)].eval(seed))
      Done.add(v)
      floodSecond(Valuation, Constraint, v, Valuation[v], Done)
...
def findIntervals(Valuation, Status, Constraints, entryPoints):
  ...
  for vertex in entryPoints:
    floodFirst(Valuation, Constraints, vertex, set([vertex]))
  ...
  for vertex in entryPoints:
    ...
    if Status[vertex] == '0':
      floodSecond(Valuation, Constraints, vertex, Interval(l, u), set())
    elif Status[vertex] == '+':
      floodSecond(Valuation, Constraints, vertex, Interval(l, '+'), set())
    elif Status[vertex] == '-':
      floodSecond(Valuation, Constraints, vertex, Interval('-', u), set())
    else:
      floodSecond(Valuation, Constraints, vertex, Interval('-', '+'), set())
