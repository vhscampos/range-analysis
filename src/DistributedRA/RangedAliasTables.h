#ifndef __RANGED_ALIAS_TABLES_H__
#define __RANGED_ALIAS_TABLES_H__
...
namespace llvm
{
	...
	struct RangedPointer 
	{
		...
		RangedPointer(Value* Pointer, APInt Lower, APInt Higher, Value* Father, Function* Func){
			pointer = Pointer; offset.setLower(Lower); offset.setUpper(Higher); father = Father; func = Func;
		}
	...
	struct RangedAliasTable
	{
		set<RangedAliasTableRow*> rows;
		...
		llvm::DenseMap<Value*, std::set<RangedAliasTableRow*> > pointer_to_rows;
	...
	class RangedAliasTables: public ModulePass
	{
		private:
			...
			void printRangedAliasTableMap();
			
		public:
			//LLVM framework methods and atributes
			static char ID;
			RangedAliasTables() : ModulePass(ID){}
			...
			llvm::DenseMap<Value*, RangedAliasTable*> getRangedAliasTableMap();
	};
}
