#define DEBUG_TYPE "ranged-alias-tables"
...
int 
RangedAliasTables::getSumBehind
...
Type* 
RangedAliasTables::getTypeInside
(Type* type, int i)
{
	if(type->isPointerTy())
...
int //returns
RangedAliasTables::getNumPrimitives //Name
(Type* type) //Parameter
{
...
RangedAliasTables::getPrimitiveLayout //Name
(Type* type) //Parameter
{
...
RangedAliasTables::printRangeAnalysis //Name
...
void //Returns Nothing
RangedAliasTables::printRangedPointerMap //Name
() //Parameters
{
	errs() << "\n-------------------------\nMemory Range Map:" << "\n";
...
void 
RangedAliasTables::printNotUsedPointers
()
{
	errs() << "\n-------------------------\nNot Used Pointers:" << "\n";
...
void //Returns Nothing 
RangedAliasTables::printPrimitiveLayouts //Name
...
void 
RangedAliasTables::printRangedAliasTableMap
()
{
	errs() << "\n-------------------------\nRanged Alias Tables:" << "\n";
	...
	{
  	errs() << "Table: " << *(i->first) << "\n";
  	for(set<RangedAliasTableRow*>::iterator ii = i->second->rows.begin(),
...
/*
* RangedAliasTables support functions
*/

llvm::DenseMap<Value*, RangedPointer*> 
RangedAliasTables::getRangedPointerMap
()
{
...
std::set<Value*> 
RangedAliasTables::getNotUsedPointers
()
{
...
RangedAliasTables::getRangedPointerSets
()
{
	return RangedPointerSets;
}

llvm::DenseMap<Value*, RangedAliasTable*> 
RangedAliasTables::getRangedAliasTableMap
()
{
	return RangedAliasTableMap;
}
...
void //Returns nothing
RangedAliasTables::getAnalysisUsage //Name
(AnalysisUsage &AU) const //Parameters
{
	AU.setPreservesAll();
	AU.addRequired<InterProceduralRA<Cousot> >();
}
char RangedAliasTables::ID = 0;
static RegisterPass<RangedAliasTables> X("ranged-alias-tables",
...
bool //Returns 
RangedAliasTables::runOnModule //Name
(Module &M) //Parameters
{
	/*
	...
	*/
	...
	i = RangedPointerSets.begin(), e = RangedPointerSets.end(); i != e; ++i)
	{
		...
		RangedAliasTableMap[i->first] = new RangedAliasTable();
		RangedAliasTableMap[i->first]->row_num = 0;
		RangedAliasTableMap[i->first]->ranged_pointers = i->second;
			...
			ee = i->second.end(); ii != ee; ++ii)
	    {
			...
			////////////////////
			//Adding row members
	    RangedAliasTableRow* table_row = new RangedAliasTableRow();
      ...
      ii != ee; ++ii)
      {
      	if((*ii)->offset.getLower().sle(higher_range) and (*ii)->offset.getUpper().sge(lower_range))
      	{
      		...
      		RangedAliasTableMap[i->first]->pointer_to_rows[(*ii)->pointer].insert(table_row);
      		mrk = true;
      	}
      }
			...
			/////////////////////
			//finishing loop turn
			RangedAliasTableMap[i->first]->rows.insert(table_row);
      if(mrk == true) RangedAliasTableMap[i->first]->row_num++;
	...
	DEBUG(printRangedAliasTableMap());
	...
	*/	
	...
	for(DenseMap<Value*, RangedAliasTable*>::iterator i = RangedAliasTableMap.begin(),
	e = RangedAliasTableMap.end(); i != e; i++)
	{
